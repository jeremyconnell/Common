...
Partial Public Class CRoleList
...
#Region "Searching (Optional)"
    ...
    'e.g. Public Function Search(ByVal nameOrId As String, ) As CRoleList
    Public Function Search(ByVal nameOrId As String) As CRoleList
        '1. Normalisation
        If IsNothing(nameOrId) Then nameOrId = String.Empty
        nameOrId = nameOrId.Trim.ToLower()
        
        '2. Start with a complete list
        Dim results As CRoleList = Me
        ...
        If Not String.IsNullOrEmpty(nameOrId) Then
            ...
            '    Dim obj As CRole = Me.GetById(id)
            '    If Not IsNothing(obj) Then
            '        results = New CRoleList(1)
            '        results.Add(obj)
            '        return results
            '    End If
            'End If
        ...
        Dim shortList As New CRoleList
        For Each i As CRole In results
    ...
    Private Function Match(ByVal name As String, ByVal obj As CRole) As Boolean
        If Not String.IsNullOrEmpty(name) Then
            If Not IsNothing(obj.RoleName) AndAlso obj.RoleName.ToLower().Contains(name) Then Return True
...
#Region "Cloning"
    Public Function Clone(ByVal target As CDataSrc) As CRoleList ', parentId As Integer) As CRoleList
        'No Transaction
        ...
        End Try
    End Function
    Public Function Clone(target as CDataSrc, txOrNull as IDbTransaction) As CRoleList ', parentId As Integer) As CRoleList
        Dim list As New CRoleList(Me.Count)
...
#Region "Export to Csv"
    ...
    Public Sub ExportToCsv(ByVal response As HttpResponse)
        ExportToCsv(response, "Roles.csv")
    End Sub
    ...
    Protected Sub ExportToCsv(ByVal sw As StreamWriter)
        Dim headings As String() = New String() {"RoleName"}
        CDataSrc.ExportToCsv(headings, sw)
        For Each i As CRole In Me
            Dim data As Object() = New Object() {i.RoleName}
