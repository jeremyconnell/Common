...
Partial Public Class CAudit_SqlList
...
#Region "Searching (Optional)"
    ...
    'e.g. Public Function Search(ByVal nameOrId As String, ) As CAudit_SqlList
    Public Function Search(ByVal nameOrId As String) As CAudit_SqlList
        '1. Normalisation
        If IsNothing(nameOrId) Then nameOrId = String.Empty
        nameOrId = nameOrId.Trim.ToLower()
        
        '2. Start with a complete list
        Dim results As CAudit_SqlList = Me
        ...
        If Not String.IsNullOrEmpty(nameOrId) Then
            ...
            '    Dim obj As CAudit_Sql = Me.GetById(id)
            '    If Not IsNothing(obj) Then
            '        results = New CAudit_SqlList(1)
            '        results.Add(obj)
            '        return results
            '    End If
            'End If
        ...
        Dim shortList As New CAudit_SqlList
        For Each i As CAudit_Sql In results
    ...
    Private Function Match(ByVal name As String, ByVal obj As CAudit_Sql) As Boolean
        If Not String.IsNullOrEmpty(name) Then
            If Not IsNothing(obj.SqlText) AndAlso obj.SqlText.ToLower().Contains(name) Then Return True
...
#Region "Cloning"
    Public Function Clone(ByVal target As CDataSrc) As CAudit_SqlList ', parentId As Integer) As CAudit_SqlList
        'No Transaction
        ...
        End Try
    End Function
    Public Function Clone(target as CDataSrc, txOrNull as IDbTransaction) As CAudit_SqlList ', parentId As Integer) As CAudit_SqlList
        Dim list As New CAudit_SqlList(Me.Count)
...
#Region "Export to Csv"
    ...
    Public Sub ExportToCsv(ByVal response As HttpResponse)
        ExportToCsv(response, "Audit_Sqls.csv")
    End Sub
    ...
    Protected Sub ExportToCsv(ByVal sw As StreamWriter)
        Dim headings As String() = New String() {"SqlId", "SqlText", "SqlCreated"}
        CDataSrc.ExportToCsv(headings, sw)
        For Each i As CAudit_Sql In Me
            Dim data As Object() = New Object() {i.SqlId, i.SqlText, i.SqlCreated}
