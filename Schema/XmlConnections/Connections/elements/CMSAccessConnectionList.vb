...
Public Class CMSAccessConnectionList : Inherits List(Of CMSAccessConnection)

#Region "Constructors (Import)"
    Public Sub New()
        MyBase.New()
    End Sub
    Public Sub New(ByVal copyFrom As IEnumerable(Of CMSAccessConnection))
        MyBase.New(copyFrom)
    End Sub
    Public Sub New(parent As CConnections, node As XmlNode)
        MyBase.New(node.ChildNodes.Count)    
        For Each i As XmlNode In CXml.ChildNodes(node, CMSAccessConnection.TAG_NAME)
            Add(New CMSAccessConnection(parent, i))
        Next
    End Sub    
#End Region
...
#Region "Public (Export)"
    Public Sub Export(node As XmlNode)
        For Each i As CMSAccessConnection In Me
            i.Export(node)
        Next
    End Sub
#End Region
