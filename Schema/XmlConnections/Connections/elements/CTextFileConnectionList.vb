...
Public Class CTextFileConnectionList : Inherits List(Of CTextFileConnection)

#Region "Constructors (Import)"
    Public Sub New()
        MyBase.New()
    End Sub
    Public Sub New(ByVal copyFrom As IEnumerable(Of CTextFileConnection))
        MyBase.New(copyFrom)
    End Sub
    Public Sub New(parent As CConnections, node As XmlNode)
        MyBase.New(node.ChildNodes.Count)    
        For Each i As XmlNode In CXml.ChildNodes(node, CTextFileConnection.TAG_NAME)
            Add(New CTextFileConnection(parent, i))
        Next
    End Sub    
#End Region
...
#Region "Public (Export)"
    Public Sub Export(node As XmlNode)
        For Each i As CTextFileConnection In Me
            i.Export(node)
        Next
    End Sub
#End Region
