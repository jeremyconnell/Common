...
Public Class CMySqlConnectionList : Inherits List(Of CMySqlConnection)

#Region "Constructors (Import)"
    Public Sub New()
        MyBase.New()
    End Sub
    Public Sub New(ByVal copyFrom As IEnumerable(Of CMySqlConnection))
        MyBase.New(copyFrom)
    End Sub
    Public Sub New(parent As CConnections, node As XmlNode)
        MyBase.New(node.ChildNodes.Count)    
        For Each i As XmlNode In CXml.ChildNodes(node, CMySqlConnection.TAG_NAME)
            Add(New CMySqlConnection(parent, i))
        Next
    End Sub    
#End Region
...
#Region "Public (Export)"
    Public Sub Export(node As XmlNode)
        For Each i As CMySqlConnection In Me
            i.Export(node)
        Next
    End Sub
#End Region
