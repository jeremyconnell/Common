...
Public Class COleDbConnectionList : Inherits List(Of COleDbConnection)

#Region "Constructors (Import)"
    Public Sub New()
        MyBase.New()
    End Sub
    Public Sub New(ByVal copyFrom As IEnumerable(Of COleDbConnection))
        MyBase.New(copyFrom)
    End Sub
    Public Sub New(parent As CConnections, node As XmlNode)
        MyBase.New(node.ChildNodes.Count)    
        For Each i As XmlNode In CXml.ChildNodes(node, COleDbConnection.TAG_NAME)
            Add(New COleDbConnection(parent, i))
        Next
    End Sub    
#End Region
...
#Region "Public (Export)"
    Public Sub Export(node As XmlNode)
        For Each i As COleDbConnection In Me
            i.Export(node)
        Next
    End Sub
#End Region
