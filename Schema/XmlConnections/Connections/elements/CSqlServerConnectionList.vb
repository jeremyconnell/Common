...
Public Class CSqlServerConnectionList : Inherits List(Of CSqlServerConnection)

#Region "Constructors (Import)"
    Public Sub New()
        MyBase.New()
    End Sub
    Public Sub New(ByVal copyFrom As IEnumerable(Of CSqlServerConnection))
        MyBase.New(copyFrom)
    End Sub
    Public Sub New(parent As CConnections, node As XmlNode)
        MyBase.New(node.ChildNodes.Count)    
        For Each i As XmlNode In CXml.ChildNodes(node, CSqlServerConnection.TAG_NAME)
            Add(New CSqlServerConnection(parent, i))
        Next
    End Sub    
#End Region
...
#Region "Public (Export)"
    Public Sub Export(node As XmlNode)
        For Each i As CSqlServerConnection In Me
            i.Export(node)
        Next
    End Sub
#End Region
