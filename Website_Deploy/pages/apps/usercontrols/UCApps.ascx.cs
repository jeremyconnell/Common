...
partial class pages_Apps_usercontrols_UCApps : UserControl
{
    #region Events
    ...
    #region Members
    private CAppList _list;
    #endregion
    
    #region Interface
    public void Display(CAppList apps)  //Complete list, not yet paged
    {
        _list = apps;
        
        //Show/Hide Columns
        colNumber.Visible = apps.Count > 0;

        //Display
        plh.Controls.Clear();
        IList sorted = null; //Fixes the numbering to reflect a user-sorted list (querystring sortBy)
        IList page = ctrlPaging.Display(apps, ref sorted); //In-Memory paging, also outputs the sorted list
        foreach (CApp i in page)
            UCApp(plh).Display(i, sorted);
    }
    #endregion
    ...
    private static pages_apps_usercontrols_UCApp UCApp(Control target)
    {
        Control ctrl = target.Page.LoadControl(CSitemap.UCApp());
        target.Controls.Add(ctrl);
        return (pages_apps_usercontrols_UCApp)ctrl;
    }
    #endregion   
