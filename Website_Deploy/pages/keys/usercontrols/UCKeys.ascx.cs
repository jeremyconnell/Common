...
partial class pages_Keys_usercontrols_UCKeys : UserControl
{
    #region Events
    ...
    #region Members
    private CKeyList _list;
    #endregion
    
    #region Interface
    public void Display(CKeyList keys)  //Complete list, not yet paged
    {
        _list = keys;
        
        //Show/Hide Columns
        colNumber.Visible = keys.Count > 0;

        //Display
        plh.Controls.Clear();
        IList sorted = null; //Fixes the numbering to reflect a user-sorted list (querystring sortBy)
        IList page = ctrlPaging.Display(keys, ref sorted); //In-Memory paging, also outputs the sorted list
        foreach (CKey i in page)
            UCKey(plh).Display(i, sorted);
    }
    #endregion
    ...
    private static pages_keys_usercontrols_UCKey UCKey(Control target)
    {
        Control ctrl = target.Page.LoadControl(CSitemap.UCKey());
        target.Controls.Add(ctrl);
        return (pages_keys_usercontrols_UCKey)ctrl;
    }
    #endregion   
