...
partial class pages_Versions_usercontrols_UCVersions : UserControl
{
    #region Events
    ...
    #region Members
    private CVersionList _list;
    #endregion

    #region Interface
    public void Display(CVersionList versions)  //Complete list, not yet paged
    {
        //Show/Hide Columns
        colNumber.Visible = versions.Count > 0;

        //Display
        plh.Controls.Clear();
        IList sorted = null; //Fixes the numbering to reflect a user-sorted list (querystring sortBy)
        IList page = ctrlPaging.Display(versions, ref sorted); //In-Memory paging, also outputs the sorted list
        foreach (CVersion i in page)
            UCVersion(plh).Display(i, sorted);
    }
    #endregion
    ...
    private static pages_versions_usercontrols_UCVersion UCVersion(Control target)
    {
        Control ctrl = target.Page.LoadControl(CSitemap.UCVersion());
        target.Controls.Add(ctrl);
        return (pages_versions_usercontrols_UCVersion)ctrl;
    }
    #endregion   
